     ...
     //   $content    =   $tag['content'];
        ...
        switch(strtoupper($type)) {
            case 'FCKEDITOR':
                ...
                break;
            case 'CKEDITOR':
                $parseStr   =	'<!-- 编辑器调用开始 --><script type="text/javascript" src="__ROOT__/Public/Js/ckeditor/ckeditor.js"></script><textarea cols="80" id="'.$id.'" name="'.$name.'" rows="10"></textarea><script>CKEDITOR.replace( "'.$id.'" );</script><!-- 编辑器调用结束 -->';
                break;
            case 'FCKMINI':
